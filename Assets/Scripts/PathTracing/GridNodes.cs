...
public class GridNodes : MonoBehaviour
{
    ...
    public void UpdateRegion(object sender, PositionArgs positionArgs)
    {
        ...
        //+ nodeDiametr * 2 - ƒелаем дл€ того, чтобы например, если маленький объект стоит между клеток, но по размерку как одна клетка, чтобы соседние все провер€л 
        //Ќу и часто колайдр меньше самого объекта
        bool walkable;
        for (int x = -rangeX; x <= rangeX; x++)
        {
            for (int y = -rangeY; y <= rangeY; y++)
            {
                // ¬ычисл€ем позицию конкретной точки в мире дл€ проверки
    ...
    public IEnumerator UpdateWalkableRoutine() //„тобы пересчитывать клетки на проходимость
    {
        int nodesProcessed = 0;
        int nodesPerFrame = 500; // —колько узлов провер€ть за один кадр
